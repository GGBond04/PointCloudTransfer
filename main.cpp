...
//    if (num_devices<1||num_devices>3)
//    {
...
//        device_indices.emplace_back(i); // 现在设备索引为 { 0, 1 }
//    }
//
//    // 修改部分
//
...
//        // 获取从属设备的校准数据
//
//        const int numSecondaryDevices = num_devices-1;
//        std::vector<k4a::calibration> secondaryCalibrations(numSecondaryDevices);
//        
//
//        // 使用 std::vector 存储次级深度到次级彩色的变换矩阵
//        std::vector<Transformation> tr_Secondary_Depth_To_Color(numSecondaryDevices);
//
//        // 使用 std::vector 存储次要设备的变换句柄
//        std::vector<k4a_transformation_t> transformationHandles(numSecondaryDevices);
//
//        transformation_handle_main = k4a_transformation_create(&main_calibration);
//        std::vector<k4a_transformation_t> transformationHandles_sub(numSecondaryDevices);
//
//        for (int i = 0; i < numSecondaryDevices; ++i) {
//            secondaryCalibrations[i] = capturer.get_subordinate_device_by_index(i).get_calibration(
//                secondary_config.depth_mode, secondary_config.color_resolution);
//
//            transformationHandles[i] = k4a_transformation_create(&secondaryCalibrations[i]);
//
//            // 获取从次级深度到次级彩色的变换矩阵
//            tr_Secondary_Depth_To_Color[i] = get_depth_to_color_transformation_from_calibration(
//                secondaryCalibrations[i]);
//
//            // 循环创建每个次要设备的变换句柄
//            k4a_transformation_t handle = k4a_transformation_create(&secondaryCalibrations[i]);
//            transformationHandles_sub.push_back(handle);
//        }
//
//
...
//
//            // 输出点云文件的相对路径
//            std::vector<std::string> outputDirectories_sub(3);
//
//            std::string outputDirectories_main = "D:\\CODE\\greenScreen\\pointcloud\\capture0";
//            outputDirectories_sub[0] = "D:\\CODE\\greenScreen\\pointcloud\\capture1";
//            outputDirectories_sub[1] = "D:\\CODE\\greenScreen\\pointcloud\\capture2";
//
//            // 获取同步的捕获帧
...
//
//
//            std::vector<k4a::image> secondaryDepthImages(3);
//            std::vector<k4a::image> secondaryColorImages(3);
//
//            for (int i = 0;i < numSecondaryDevices;i++)
//            {
//                //获取从属1深度图像和从属1彩色图像
//                secondaryDepthImages[i] = captures[i + 1].get_depth_image();
//                secondaryColorImages[i] = captures[i + 1].get_color_image();
//            }
//         
//
//            // 保存点云文件
//            std::string fileName_main= outputDirectories_main + "\\" + std::to_string(count) + ".ply";
//            std::vector<std::string> fileName_sub(3);
//            //fileName[0] = outputDirectories[0] + "\\" + std::to_string(count) + ".ply";
//            fileName_sub[0] = outputDirectories_sub[0] + "\\" + std::to_string(count) + ".ply";
//            fileName_sub[1] = outputDirectories_sub[1] + "\\" + std::to_string(count) + ".ply";
//
//            // 配准矩阵
//            std::vector<std::string> matrix(3);
//            matrix[0] = "D:\\CODE\\greenScreen\\pointcloud\\capture1\\1to0_matrix.txt";
//            matrix[1] = "D:\\CODE\\greenScreen\\pointcloud\\capture2\\2to0_matrix.txt";
//
//            save_point_cloud_new(transformation_handle_main, main_depth_image, main_color_image, fileName_main, ""); //空字符串
//            std::cout << "main_image_done" << std::endl;
//
//            for(int i=0;i<numSecondaryDevices;i++)
//            save_point_cloud_new(transformationHandles_sub[i], secondaryDepthImages[i], secondaryColorImages[i], fileName_sub[i], matrix[i]);
//            count++;
//            //break;
//
//
...
//    std::cout << "tranformation_helpers_write_point_cloud_done" << std::endl;
//
//    // 直播
...
int main(int argc, char** argv)
{
    ...
    if (num_devices == 1) {
            ...
            greenscreen_duration)
        {
            ...
            std::string outputDirectory0 = "D:\\CODE\\greenScreen\\pointcloud\\capture0";
    ...
    else if (num_devices == 2)
    {
            ...
            greenscreen_duration)
        {
            ...
            // pointcloud 定义输出点云文件的目录
            std::string outputDirectory0 = "D:\\CODE\\greenScreen\\pointcloud\\capture0";
            std::string outputDirectory1 = "D:\\CODE\\greenScreen\\pointcloud\\capture1";
            ...
            // 配准矩阵
            std::string matrix1 = "D:\\CODE\\greenScreen\\pointcloud\\capture1\\1to0_matrix.txt";
